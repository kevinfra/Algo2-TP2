...
\begin{Estructura}{DiccNat}[estr]
	\begin{Tupla}[estr]
		...
		\tupItem{listaIterable}{lista(puntero($\alpha$))}
	\end{Tupla}
\end{Estructura}
...
\begin{enumerate}
	...
	\item Todo significado de la tabla es apuntado por un elemento de listaIterable.
...
\Rep[estr][e]{
	...
	($\forall$ l : lista(tupla(nat, $\alpha$)))(esta?(l, e.tabla) 
		...
		\\ $\Rightarrow$ ($\exists$ p : puntero($\alpha$)($\&$significado(t) $=_{obs}$ p $\land$ esta?(p, e.listaIterable)))))
	$\\ \land \\$
