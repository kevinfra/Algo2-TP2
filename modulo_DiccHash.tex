...
\Rep[estr][e]{
	\\
	($\forall$ $l$ : lista(tupla(nat, $\alpha$))) (( $\exists$ $k$ : nat ) ((e.tabla[k] = l) $\Rightarrow$ ($\not \exists$ $q$ : nat) (k $\neq$ q $\land$ e.tabla[q] = l)) $\land$ ($\forall$ $t1$, $t2$ : tupla(nat, $\alpha$)) (esta?($t1$,l) $\land$ esta?($t2$,l) $\Rightarrow$ ($\not \exists$ n,m : nat) ($t1$.clave = n $\land$ $t2$.clave = m $\land_L$ n = m) ))
	$\\ \land \\$
	...
	$\\ \land \\$
	long(e.vectorOrdenClaves) $=$ long(e.tabla) $\land$ ($\forall$ l : lista(tupla(nat, $\alpha$)))(esta?(l, e.tabla)
		\\ $\Rightarrow$ ($\forall$ t : tupla(nat, $\alpha$)) (esta?(t, l) \\ $\Rightarrow$ ($\forall$ n : nat) (n $=$ t.clave)) \\$\Rightarrow_L$
		($\exists$ p : puntero($\alpha$)) ($\&$t.significado $=$ p) $\land$ esta?((t.clave, p), e.vectorOrdenClaves))
}\mbox{}
