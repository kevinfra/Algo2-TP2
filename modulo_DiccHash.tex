...
\begin{Estructura}{DiccNat}[estr]
	\begin{Tupla}[estr]
		\tupItem{tabla}{vector(lista(tupla(agente, $\alpha$)))}
		\tupItem{\\ listaIterable}{lista(puntero(tupla(nat,$\alpha$)))}
	\end{Tupla}
\end{Estructura}
...
\begin{Estructura}{itDiccNat($\alpha$)}[itDiccNat donde itDiccNat es $\newline$ \- \- \- \- vector(tupla $<$clave : nat, significado : $\alpha>$))]

\end{Estructura}

\begin{enumerate}
	\item La suma del largo de las listas siguientes($it$) y anteriores($it$) es igual al numero de claves del diccionario.
	\item Todas las claves que estan en el diccionario, son vistas por el iterador.
\end{enumerate}

\subsubsection{Invariante de Representación}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% A SEGUIR CON EL REP Y EL ABS!!!!!!

\Rep[itDiccNat][it]{
	\\
			($\forall$ $d$ : DiccNat($\alpha$)) longitud(anteriores($it$)) + longitud(siguientes($it$)) = $\#$claves(d)
			%\- \- $\Rightarrow$ ($\forall$ $t_1$,$t_2$ : tupla(nat, $\alpha$) esta?($t_1$,l) $\land$ esta?($t_2$,l))) $\\$
			%\- \- $\Rightarrow_{L}$	($\forall$ $n_1$, $n_2$ : nat) $n_1 =_{obs} t_1$.clave $\land n_2 =_{obs} \ t_2$.clave $\land$ $t_1$ $\not =$ $t_2$) $\Rightarrow$ $n_1 \not = n_2$ $\\$
			%$\land$ $\\$
