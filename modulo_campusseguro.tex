...
'\alg{iIngresarEstudiante}{\In{e}{nombre}, \In{pos}{posicion}, \Inout{cs}{estr} } {}'
	if CantHippiesVecinos(vecinos(pos, $cs$.campus), $cs$)$< 2$  then '\ote{1}'
		Definir($cs$.estudiantes, e, pos) '\ote{$|n_a|$}'
	end if '\ote{$|n_a|$}'
	
	Lista(nombre, posicion): listaHippies $\leftarrow$ 
		$\leftarrow$ HippiesRodeados(pos, vecinos(pos, $cs$.campus), $cs$) '\ote{$|n_a|$}'

	if Longitud(listahippies) $>$ 0 then '\ote{1}'
		itLista(nombre): itL $\leftarrow$ CrearIt(listahippies) '\ote{1}'
		Definir($cs$.estudiantes, Siguiente(it).nombre, Siguiente(it).posicion) '\ote{$|n_a|$}'
		Eliminar($cs$.hippies, Siguiente(it).nombre) '\ote{$|n_a|$}'
	end if '\ote{$|n_a|$}'
