...
'\alg{iIngresarEstudiante}{\In{e}{nombre}, \In{pos}{posicion}, \Inout{cs}{estr} } {}'
	...
	conj(nombre, posicion): conjHippiesRodEst $\leftarrow$ 
		$\leftarrow$ HippiesRodeadosEstudiantes(pos, vecinos(pos, $cs$.campus), $cs$) '\ote{$|n_a|$}'

	if Cardinal(conjHippiesRodEst) $>$ 0 then '\ote{1}'
		itConj(nombre, posicion): itConj $\leftarrow$ CrearIt(conjHippiesRodEst) '\ote{1}'
		Definir($cs$.estudiantes, Siguiente(itConj).nombre, 
			Siguiente(itConj).posicion) '\ote{$|n_a|$}'
...
\begin{lstlisting}[mathescape]
'\alg{HippiesRodeadosEstudiantes}{\In{pos}{posicion}, \In{c}{conj(posicion}, \In{cs}{estr}}{conj(nombre, posicion)}'
	itConj(posicion): itC $\leftarrow$ CrearIt(c) '\ote{1}'
