...
'\alg{icomenzarRastrillaje}{\In{c}{campus}, \In{d}{diccNat(agente, datosAgente)}}{estr}'
	...
	res.posicionesAgente $\leftarrow$ vectorizarPos(d,filas(c),columnas(c)) $\textit{\textbf{(c2)}}$ '\ote{(f$*$c)$^3$ + $N_a$}'
	...
	end while $\textit{\textbf{(c4)}}$ '\ote{(f*c)*2k}'
...
'\alg{vectorizarPos}{\In{d}{diccNat(agente, datosAgente)}, \In{f}{nat}, \In{c}{nat}}{vector(As)}'
	...
	while i $<$ f$*$c do '\ote{1}'
		...
		i $\leftarrow$ i$+1$ '\ote{1}'
	end while $\textit{\textbf{(c1)}}$ '\ote{ ((f$*$c) $*$ k) }'
...
'\ofi{O((f*c)*\sum_{k=0}^{f*c - 1}(k) + N_a) = O((f*c)*(f*c-1)*(f*c)/2 + N_a) \leq O((f*c)^3 + N_a)}'
\end{lstlisting}
...
'\alg{SancionarAgentes}{\In{c}{conj(As)}, \Inout{cs}{estr}}{}'
	...
	while HaySiguiente(itC) do '\ote{1}'
		...
		if Siguiente(cs.masVigilante.datos).cantSanc $<$
			$<$ Siguiente(Siguiente(itC).datos).cantSanc then '\ote{1}'
