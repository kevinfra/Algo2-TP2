...
'\alg{iIngresarEstudiante}{\In{e}{nombre}, \In{pos}{posicion}, \Inout{cs}{estr} } {}'
	...
	conj(nombre, posicion): conjHippiesRod $\leftarrow$ 
		$\leftarrow$ HippiesRodeados(pos, vecinos(pos, $cs$.campus), $cs$) '\ote{$|n_a|$}'

	if Cardinal(conjHippiesRod) $>$ 0 then '\ote{1}'
		itConj(nombre): itConj $\leftarrow$ CrearIt(conjHippiesRod) '\ote{1}'
		Definir($cs$.estudiantes, Siguiente(itConj).nombre, Siguiente(itConj).posicion) '\ote{$|n_a|$}'
		Eliminar($cs$.hippies, Siguiente(itConj).nombre) '\ote{$|n_a|$}'
	end if '\ote{$|n_a| + |n_a|$}'

'\ofi{O(2|n_a|) + O(|n_a|) + O(2|n_a|) = O(5|n_a|) = O(|n_a|)}'
\end{lstlisting}
...
\begin{lstlisting}[mathescape]
'\alg{HippiesRodeados}{\In{pos}{posicion}, \In{c}{conj(posicion}, \In{cs}{estr}}{conj(nombre, posicion)}'
